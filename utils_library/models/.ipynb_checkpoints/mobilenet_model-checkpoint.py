...
class ModelMaker:
    ...
    def _make_mobilenetV2_conv(self, x, input_c, output_c, name, s=1):
        initializer = tf.initializers.VarianceScaling(scale=2.0)
        
        # expand part
        x = tf.keras.layers.Conv2D(input_c*6, 1, use_bias=False, kernel_initializer=initializer, name=name+'_expand_conv')(x)
    ...
    def create_models(self):
        ...
        # stage 00 (i know)
        stage00_output = self._make_mobilenet_input_model(input_tensor)
        # stage 0 2conv)
...
class ModelDatasetComponent:
    def __init__(self, config, id_flag=False):
        self.INCLUDE_MASK = config.INCLUDE_MASK
    ...
    def place_training_labels(self, elem):
        ...
        configuration = (inputs, (paf_tr, paf_tr, paf_tr, paf_tr, kpt_tr, kpt_tr))
        
        if self.ID_FLAG:
            configuration = (configuration, elem['id'])
        
        return configuration  # this should match the model outputs, and is different for each model
